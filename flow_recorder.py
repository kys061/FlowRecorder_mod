...
################################################################################
FIELDS = [
    'distress', 'geolocation', 'autonomous_system',
    'retransmissions', 'round_trip_time', 'timeouts udp_jitter',
    'red_threshold_discards', 'in_control', 'delay', 'rtt_server', 'rtt_client',
]
...
for i in range(len(INTERFACE_LIST)):
    CMD.append("echo \'show int {} flows top {} by average_rate select {}\' |sudo {} {}:{}@{}".format(
        INTERFACE_LIST[i], TOP_NUM, selected, STM_SCRIPT_PATH, USERNAME, PASSWORD, DOMAIN))
# RECORD_CMD_TYPE:2
CMD_BY_SOURCEHOST = []
for host in HOST:
    for intf in INTERFACE_LIST:
        for d_intf in D_INTERFACE_LIST['internal']:
            if intf == d_intf:
                CMD_BY_SOURCEHOST.append("echo \'show interface {} flows with source_host={} select {}\' |sudo {} {}:{}@{}".format(
                    intf, host, selected, STM_SCRIPT_PATH, USERNAME, PASSWORD, DOMAIN))
# RECORD_CMD_TYPE:2
CMD_BY_DESTHOST = []
for host in HOST:
    for intf in INTERFACE_LIST:
        for d_intf in D_INTERFACE_LIST['external']:
            if intf == d_intf:
                CMD_BY_DESTHOST.append("echo \'show interface {} flows with dest_host={} select {}\' |sudo {} {}:{}@{}".format(
                    intf, host, selected, STM_SCRIPT_PATH, USERNAME, PASSWORD, DOMAIN))
################################################################################
